...
function Decoder(bytes, port) {
    ...
    switch (port) {
        ...
        case 2:
            decoded = rakMinimalDataDecode(bytes);
            break;
...
// decode Minimal mapping data to object
function rakMinimalDataDecode(bytes){
    var myObj = {};
    if (bytes.length === 8) 
    {
        var latitude = Number(((bytes[0] << 24 | bytes[1] << 16 | bytes[2] << 8) * 1e-7).toFixed(7));
        var longitude = Number(((bytes[3] << 24 | bytes[4] << 16 | bytes[5] << 8 ) * 1e-7).toFixed(7));
        myObj.altitude = Number(((bytes[6] << 8 | bytes[7])).toFixed(0));
  ...
  while (str.length > 4) {
    ...
    switch (flag) {
      ...
      case 0x0267:// Temperature
        ...
        str = str.substring(8);
        break;
      case 0x0187: // GPS Minimal
        var latitude = parseFloat((parseTriple(str.substring(4, 10), 16)*256 * 0.0000001).toFixed(7));
        var longitude = parseFloat((parseTriple(str.substring(10, 16), 16)*256 * 0.0000001).toFixed(7));
        myObj.altitude = parseFloat((parseTriple(str.substring(16, 20), 16)).toFixed(0)); // Meter
        myObj.location = "(" + latitude + "," + longitude + ")";
        myObj.location = "(" + latitude + "," + longitude + ")";
...
//        break;
      ...
      case 0x0802:// Battery Voltage
        myObj.BATTERY_VOLTAGE = parseFloat((parseShort(str.substring(4, 8), 16) * 0.01).toFixed(2));// + "V";//unit:V
        str = str.substring(8);
        break;
